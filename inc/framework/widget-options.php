...
class Atframework_Widget_Options {
  ...
  ?>
    ...
    <div class="at-styling-options">
      <h4><?php _e( 'Styling', 'PointJupiter' ); ?><span><?php _e( 'Click to expand', 'PointJupiter' ); ?></span></h4>
      <div class="at-styling-inner">
        <em style="font-size:12px;"><?php _e( 'Options found in this panel apply only to this widget.', 'PointJupiter' ); ?></em>
        <p><label for="<?php echo $widget->get_field_id('sectionId'); ?>"><?php _e('Section ID', 'PointJupiter'); ?><br></label>
        ...
        <p><label for="<?php echo $widget->get_field_id('background_color'); ?>"><?php _e('Background color', 'PointJupiter'); ?><br></label>
        ...
        <p><label for="<?php echo $widget->get_field_id('text_color'); ?>"><?php _e('Text color', 'PointJupiter'); ?><br></label>
        ...
        <p><label for="<?php echo $widget->get_field_id('widget_title_color'); ?>"><?php _e('Widget title color', 'PointJupiter'); ?><br></label>
        ...
        <p><label for="<?php echo $widget->get_field_id('image_uri'); ?>"><?php _e('Background image URL', 'PointJupiter'); ?><br></label>
        ...
        <p>
        <label for="<?php echo $widget->get_field_id('column_width'); ?>"><?php _e( 'Widget width:', 'PointJupiter' ); ?></label>
    	  <select name="<?php echo $widget->get_field_name('column_width'); ?>" id="<?php echo $widget->get_field_id('column_width'); ?>" class="widefat">
      	<?php
      	$options = array(
      		'full' 	=> __( 'Full Width', 'PointJupiter' ), 
      		'half' 	=> __( '1/2 Width', 'PointJupiter' ), 
      		'third' => __( '1/3 Width', 'PointJupiter' ), 
      	); 
        ...
        <p><label for="<?php echo $widget->get_field_id('padding'); ?>"><?php _e('Top/bottom padding [px]', 'PointJupiter'); ?><br></label>
        ...
        <label for="<?php echo $widget->get_field_id( 'no_container' ); ?>"><?php _e( 'Remove container?', 'PointJupiter' ); ?></label></p>        
      </div>   
    </div>
  ...
  public function widget_output( $params ) {
    ...
    if (isset($wp_registered_widgets[$id]['callback'][0]) && is_object($wp_registered_widgets[$id]['callback'][0])) {
      ...
      $widget_title_color = isset($instance['widget_title_color']) ? $instance['widget_title_color'] : null;      
	    $column_width       = isset( $instance['column_width'] ) ? esc_attr($instance['column_width']) : __( 'Full Width', 'PointJupiter' );
      $image_uri          = isset( $instance['image_uri'] ) ? esc_url($instance['image_uri']) : '';
      $padding            = isset( $instance['padding'] ) ? intval($instance['padding']) : '100';
  ...
  public function widget_color_picker( $hook ) {
    ...
    wp_enqueue_script( 'PointJupiter-picker', get_template_directory_uri() . '/inc/framework/js/colorpicker.js', array( 'wp-color-picker' ), false, true );  
  }
