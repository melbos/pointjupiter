<?php
/**
 * PointJupiter functions and definitions.
 *
...
if ( ! function_exists( 'PointJupiter_setup' ) ) :
/**
 ...
 */
function PointJupiter_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on PointJupiter, use a find and replace
	 * to change 'PointJupiter' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'PointJupiter', get_template_directory() . '/languages' );
	...
	add_image_size('PointJupiter-large-thumb', 700);
	add_image_size('PointJupiter-medium-thumb', 520);
	add_image_size('PointJupiter-small-thumb', 360);
	add_image_size('PointJupiter-project-thumb', 500, 310, true);
	add_image_size('PointJupiter-client-thumb', 250);
	add_image_size('PointJupiter-testimonial-thumb', 100);

	// This theme uses wp_nav_menu() in one location.
	register_nav_menus( array(
		'primary' 	=> esc_html__( 'Primary', 'PointJupiter' ),
		'footer' 	=> esc_html__( 'Footer', 'PointJupiter' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'PointJupiter_custom_background_args', array(
...
add_action( 'after_setup_theme', 'PointJupiter_setup' );
 ...
 */
function PointJupiter_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'PointJupiter_content_width', 640 );
}
add_action( 'after_setup_theme', 'PointJupiter_content_width', 0 );
 ...
 */
function PointJupiter_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'PointJupiter' ),
		'id'            => 'sidebar-1',
		'description'   => '',
	...
	foreach($pages as $page){
		register_sidebar( array(
			'name'          => esc_html__( 'Page - ', 'PointJupiter' ) . $page->post_title,
			'id'            => 'widget-area-' . strtolower($page->post_name),
			'description'   => esc_html__( 'Use this widget area to build content for the page: ', 'PointJupiter' ) . $page->post_title,
...
add_action( 'widgets_init', 'PointJupiter_widgets_init' );

//Homepage widgets
$PointJupiter_widgets = array('services', 'skills', 'facts', 'employees', 'projects', 'testimonials', 'clients', 'posts');
foreach ( $PointJupiter_widgets as $PointJupiter_widget) {
	locate_template( '/inc/framework/widgets/front-' . $PointJupiter_widget . '.php', true, false );
}
 ...
 */
function PointJupiter_scripts() {
	wp_enqueue_style( 'PointJupiter-style', get_stylesheet_uri() );
	...
	wp_enqueue_style( 'PointJupiter-body-fonts', esc_url($body_url) ); 
	
	wp_enqueue_style( 'PointJupiter-headings-fonts', esc_url($headings_url) ); 	

	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );	

	wp_enqueue_script( 'PointJupiter-main', get_template_directory_uri() . '/js/main.js', array('jquery'), '', true );

	wp_enqueue_script( 'PointJupiter-scripts', get_template_directory_uri() . '/js/scripts.min.js', array('jquery'), '20210510', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

	if ( PointJupiter_blog_layout() == 'masonry-layout' && (is_home() || is_archive()) ) {
		wp_enqueue_script( 'PointJupiter-masonry-init', get_template_directory_uri() . '/js/masonry-init.js', array('masonry'), '', true );		
	}

	wp_enqueue_script( 'PointJupiter-html5shiv', get_template_directory_uri() . '/js/html5shiv.js', array(), '', true );
    wp_script_add_data( 'PointJupiter-html5shiv', 'conditional', 'lt IE 9' );

}
add_action( 'wp_enqueue_scripts', 'PointJupiter_scripts' );

/**
 * Enqueue Bootstrap
 */
function PointJupiter_enqueue_bootstrap() {
	wp_enqueue_style( 'bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'PointJupiter_enqueue_bootstrap', 9 );


/**
 * Customizer styles
 */
function PointJupiter_customizer_styles($hook) {
    if ( ( 'customize.php' != $hook ) && ( 'widgets.php' != $hook ) ) {
        return;
    } 	
	wp_enqueue_style( 'PointJupiter-customizer-styles', get_template_directory_uri() . '/inc/framework/css/customizer.css' );	
}
add_action( 'admin_enqueue_scripts', 'PointJupiter_customizer_styles' );

/**
 * Blog layout
 */
function PointJupiter_blog_layout() {
 ...
 */
function PointJupiter_category_label($title) {
    if ( is_category() ) {
...
add_filter('get_the_archive_title', 'PointJupiter_category_label');

/**
 * Header image check
 */
function PointJupiter_has_header() {
	$front_header = get_theme_mod('front_header_type' ,'image');
	$site_header = get_theme_mod('site_header_type', 'nothing');
	global $post;
	if ( !is_404() || !is_search() ) {
		$single_toggle = get_post_meta( $post->ID, '_PointJupiter_single_header_shortcode', true );
	} else {
		$single_toggle = false;
	}
 ...
 */
function PointJupiter_fullwidth_singles($classes) {
...
add_filter('body_class', 'PointJupiter_fullwidth_singles');

/**
 * Polylang compatibility
 */
if ( function_exists('pll_register_string') ) :
function PointJupiter_polylang() {
	pll_register_string('Header text', get_theme_mod('header_text'), 'PointJupiter');
	pll_register_string('Header subtext', get_theme_mod('header_subtext'), 'PointJupiter');
	pll_register_string('Header button', get_theme_mod('header_button'), 'PointJupiter');
}
add_action( 'admin_init', 'PointJupiter_polylang' );
endif;

/**
 * Header text
 */
function PointJupiter_header_text() {
 ...
 * Site branding
 */
if ( ! function_exists( 'PointJupiter_branding' ) ) :
function PointJupiter_branding() {
 ...
 */
if ( ! function_exists( 'PointJupiter_footer_branding' ) ) :
function PointJupiter_footer_branding() {
 ...
 * Footer contact
 */
if ( ! function_exists( 'PointJupiter_footer_contact' ) ) :
function PointJupiter_footer_contact() {
 ...
 * Clearfix posts
 */
function PointJupiter_clearfix_posts( $classes ) {
	$classes[] = 'clearfix';
	return $classes;
}
add_filter( 'post_class', 'PointJupiter_clearfix_posts' );

/**
 * Excerpt length
 */
function PointJupiter_excerpt_length( $length ) {
  $excerpt = get_theme_mod('exc_length', '40');
  return absint($excerpt);
}
add_filter( 'excerpt_length', 'PointJupiter_excerpt_length', 99 );

/**
* Footer credits
*/
function PointJupiter_footer_credits() {
	echo '<a href="' . esc_url( __( 'https://wordpress.org/', 'PointJupiter' ) ) . '">';
		printf( __( 'Powered by %s', 'PointJupiter' ), 'WordPress' );
	echo '</a>';
	echo '<span class="sep"> | </span>';
	printf( __( 'Theme: %2$s by %1$s.', 'PointJupiter' ), 'aThemes', '<a href="http://athemes.com/theme/PointJupiter" rel="nofollow">PointJupiter</a>' );
}
add_action( 'PointJupiter_footer', 'PointJupiter_footer_credits' );

function PointJupiter_remove_page_template() {

	if ( !function_exists('PointJupiter_pro_load_widgets') ) {
...
add_action('admin_footer', 'PointJupiter_remove_page_template', 10);

/*
 * Disable Elementor globals
 */
function PointJupiter_disable_elementor_globals() {

	update_option( 'elementor_disable_color_schemes', 'yes' );
	update_option( 'elementor_disable_typography_schemes', 'yes' );	

}
add_action('after_switch_theme', 'PointJupiter_disable_elementor_globals');
...
function PointJupiter_editor_styles() {
	wp_enqueue_style( 'PointJupiter-block-editor-styles', get_theme_file_uri( '/PointJupiter-gutenberg-editor-styles.css' ), '', '1.0', 'all' );
	...
	wp_add_inline_style( 'PointJupiter-block-editor-styles', $custom );	

}
add_action( 'enqueue_block_editor_assets', 'PointJupiter_editor_styles' );
